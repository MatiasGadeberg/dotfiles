...
ssh-add ~/.ssh/github ~/.ssh/ado-nn ~/.ssh/ado-nne 2>/dev/null
...
## function for dynamically adding commit messages to git add, commit push
gpa() {
    git add .
    if [ -n "$1" ]
    then
...
alias dps="docker ps"
alias dnpm='docker run -it -v `pwd`:/app --rm personal-setup_npm'
